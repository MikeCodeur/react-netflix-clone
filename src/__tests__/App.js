...
beforeEach(async () => await authNetflix.logout())
afterEach(async () => {
...
//bonus-2
test('rendu de route la list', async () => {
  const route = `/series`
  window.history.pushState({}, 'Test page2', route)
  ...
  //   screen.getByRole('button', {name: /Plus d'infos/i}),
  // )
  //screen.debug()
  await waitForElementToBeRemoved(() => screen.getByRole('alert'))
  expect(screen.queryByText(/Connexion/i)).not.toBeInTheDocument()
